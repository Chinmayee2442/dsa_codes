...
void DFS(struct Graph* graph, int startVertex) {
  ...
  int traversalIndex = 0; // Index to store the traversal order

  while (!isEmpty(stack)) {
    int currentVertex = pop(stack);
    graph->traversalOrder[traversalIndex++] = currentVertex; // Store the visited vertex in the traversal order array
...
struct Graph* createGraph(int vertices) {
  ...
  graph->traversalOrder = malloc(vertices * sizeof(int));
